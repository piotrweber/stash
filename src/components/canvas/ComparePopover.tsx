...
export function ComparePopover({ itemIds, containerW, containerH, onClose }: ComparePopoverProps) {
  ...
  const left = Math.max(8, (containerW - POPOVER_W) / 2)
  const top = Math.max(8, (containerH - 480) / 2)
  const maxH = containerH - top - 16
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        <SwapRow
          label="Image"
          left={item1.imagePath
            ? <img src={item1.imagePath} alt={item1.name} className="w-14 h-14 object-contain rounded bg-gray-100" />
            : <div className="w-14 h-14 bg-gray-100 rounded flex items-center justify-center text-xl text-gray-400">{item1.name.charAt(0).toUpperCase()}</div>
          }
          right={item2.imagePath
            ? <img src={item2.imagePath} alt={item2.name} className="w-14 h-14 object-contain rounded bg-gray-100" />
            : <div className="w-14 h-14 bg-gray-100 rounded flex items-center justify-center text-xl text-gray-400">{item2.name.charAt(0).toUpperCase()}</div>
          }
          onSwap={() => swap(it => it.imagePath, (_, v) => ({ imagePath: v }))}
        />
        <SwapRow
          label="Name"
          ...
          onSwap={() => swap(it => it.name, (_, v) => ({ name: v }))}
        />
        <SwapRow
          label="Description"
          ...
          onSwap={() => swap(it => it.description, (_, v) => ({ description: v }))}
        />
...
function SwapRow({ label, left, right, onSwap }: {
  label: string
